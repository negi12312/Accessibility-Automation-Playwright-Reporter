...
test.describe('Neom Next Page Accessibility Test', () => {
    ...
    async function generateComprehensiveReport(accessibilityScanResults: any, page: import('@playwright/test').Page, label: string, url: string, pageTitle: string = '') {
        ...
        <head>
            ...
            <title>Neom Next Page Accessibility Report - ${label}</title>
        ...
        <body>
            <div class="container mt-4">
                <div class="summary-stats">
                    <h1>🔍 Neom Next Page Accessibility Report</h1>
                    <p class="lead">Page: ${pageTitle || 'Next Page After Login'}</p>
                ...
                <div class="page-info">
                    ...
                    <p><strong>Page Title:</strong> ${pageTitle || 'Next Page After Login'}</p>
                    <p><strong>Current URL:</strong> ${url}</p>
                    <p><strong>Navigation Flow:</strong> Login → Next Page → Accessibility Scan</p>
                </div>
                ...
                <div class="test-info">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            <p><strong>Server:</strong> neom.btsmomenta.com</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-4">
                    <h3>🚨 Accessibility Violations Found</h3>
                    <div class="alert alert-warning">
                        <strong>⚠️ ${accessibilityScanResults.violations.length} accessibility issues detected!</strong>
                        Please review the detailed violations below and implement fixes to improve accessibility compliance.
                    </div>
                ...
                ` : '<div class="alert alert-success mt-4"><h4>✅ No accessibility violations found!</h4><p>Excellent! This page meets accessibility standards and provides a good user experience for all users.</p></div>'}
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <div class="card text-center">
                                <div class="card-body">
                                    <h5 class="card-title text-danger">${accessibilityScanResults.violations.length}</h5>
                        ...
                        <div class="col-md-4">
                            <div class="card text-center">
                                <div class="card-body">
                                    <h5 class="card-title text-success">${accessibilityScanResults.passes.length}</h5>
                        ...
                        <div class="col-md-4">
                            <div class="card text-center">
                                <div class="card-body">
                                    <h5 class="card-title text-warning">${accessibilityScanResults.incomplete.length}</h5>
        ...
        const reportPath = path.join('build/reports', `neom-next-page-accessibility-report-${label.replace(/\s+/g, '-').toLowerCase()}.html`);
        fs.writeFileSync(reportPath, reportHTML);
        console.log(`📄 Comprehensive report generated: ${reportPath}`);
        return reportPath;
    }

    test('Neom Next Page Accessibility Test with Login', async ({ page }) => {
        // Set longer timeout for the entire test
        test.setTimeout(120000); // 2 minutes
        
        console.log('🚀 Starting Neom Next Page Accessibility Test...');
        ...
        // Step 4: Navigate to the next page and wait for all elements to load
        ...
        // Try to navigate to the next logical page (dashboard or main area)
        ...
        await page.waitForLoadState('networkidle');
        await page.waitForTimeout(8000); // Extended wait for dynamic content
        
        // Try to wait for specific elements that indicate the page is fully loaded
        try {
            // Wait for common page elements that indicate full loading
            ...
            console.log('ℹ️ No specific loading indicators found, proceeding with scan');
        }
        
        const finalUrl = page.url();
        const pageTitle = await page.title();
        console.log(`✅ Successfully navigated to next page: ${finalUrl}`);
        console.log(`📄 Page title: ${pageTitle}`);

        // Step 5: Take screenshot of the next page
        console.log('📸 Taking screenshot of the next page...');
        await takeScreenshot(page, 'next-page');

        // Step 6: Run comprehensive accessibility scan on the next page
        console.log('🔍 Running comprehensive axe DevTool accessibility scan on the next page...');
        const nextPageAccessibilityResults = await new AxeBuilder({ page })
            .withTags(['wcag2a', 'wcag2aa', 'wcag21aa', 'wcag21a'])
            .disableRules(['page-has-heading-one']) // Exclude best-practice rule for h1 heading
            .analyze();

        console.log(`📈 Next page scan results: ${nextPageAccessibilityResults.violations.length} violations found`);
        
        // Step 7: Generate comprehensive HTML report with screenshots
        console.log('📊 Generating comprehensive HTML report...');
        await generateComprehensiveReport(nextPageAccessibilityResults, page, 'next-page', finalUrl, pageTitle);

        // Generate axe-html-reporter format report
        const axeReportHTML = createHtmlReport({
            results: nextPageAccessibilityResults,
            options: {
                projectKey: "NeomNextPageAccessibilityTest"
            },
        });

        fs.writeFileSync("build/reports/neom-next-page-axe-report.html", axeReportHTML);
        console.log('✅ Axe HTML report generated: build/reports/neom-next-page-axe-report.html');

        // Print final summary
        console.log('\n📋 NEOM NEXT PAGE ACCESSIBILITY TEST SUMMARY:');
        console.log('='.repeat(70));
        console.log(`Next Page URL: ${finalUrl}`);
        console.log(`Page Title: ${pageTitle}`);
        console.log(`Login Credentials: test_cu2@neom.com / ABab22$`);
        console.log(`Total Violations: ${nextPageAccessibilityResults.violations.length}`);
        console.log(`Serious Issues: ${nextPageAccessibilityResults.violations.filter(v => v.impact === 'serious').length}`);
        console.log(`Moderate Issues: ${nextPageAccessibilityResults.violations.filter(v => v.impact === 'moderate').length}`);
        console.log(`Minor Issues: ${nextPageAccessibilityResults.violations.filter(v => v.impact === 'minor').length}`);
        console.log(`Passes: ${nextPageAccessibilityResults.passes.length}`);
        console.log(`Incomplete: ${nextPageAccessibilityResults.incomplete.length}`);
        console.log('='.repeat(70));
        console.log('📁 Reports generated in: build/reports/');
        console.log('   - neom-next-page-accessibility-report-next-page.html (Main report with screenshots)');
        console.log('   - neom-next-page-axe-report.html (Axe format report)');
        console.log('📸 Screenshots saved in: build/reports/screenshots/');
        
        if (nextPageAccessibilityResults.violations.length > 0) {
            console.log('\n⚠️  Accessibility violations found! Please review the reports for details.');
            console.log('\n🚨 Violations Summary:');
            nextPageAccessibilityResults.violations.forEach((violation, index) => {
        ...
        // expect(nextPageAccessibilityResults.violations).toEqual([]);
    });
});
